...
async function sendCurrentResults(){
  ...
  if(!s.name || !s.course){
    if(status){status.textContent="Bitte Name und Kurs eingeben.";status.className="small saveBad";}
    return;
  }
  if(!s.sheetUrl){
  ...
  const payload={
    ...
    timeSeconds:Math.round((Date.now()-phaseStart)/1000),
  ...
  try{
    ...
    if(status){status.textContent="Ergebnisse wurden gesendet.";status.className="small saveOk";}
  }catch(e){
    if(status){status.textContent="Senden fehlgeschlagen. Prüfe die Web-App-URL.";status.className="small saveBad";}
  }
}
...
function render(){let t=getTheme(),m=document.getElementById('modeSelect').value; phaseStart=Date.now(); lastMode=m;let h=`<section class="card"><span class="badge">${esc(t.title)}</span><h2>${esc(t.title)}</h2></section>`; if(m==="training")h+=training(t); if(m==="families")h+=familiesMode(t); if(m==="prep")h+=prepMode(t); if(m==="memory")h+=memory(t); if(m==="crossword")h+=crossword(t); if(m==="exam")h+=exam(t); if(m==="mini")h+=mini(t); document.getElementById('content').innerHTML=h; updateScore(); if(m==="memory")initMemory(t); if(m==="training")initLetterTasks();}
...
fillSelect();loadSettings();render();
