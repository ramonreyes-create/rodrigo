...
function render(){let t=getTheme(),m=document.getElementById('modeSelect').value; phaseStart=Date.now(); phaseStart=Date.now(); lastMode=m;let h=`<section class="card"><span class="badge">${esc(t.title)}</span><h2>${esc(t.title)}</h2></section>`; if(m==="training")h+=training(t); if(m==="families")h+=familiesMode(t); if(m==="prep")h+=prepMode(t); if(m==="memory")h+=memory(t); if(m==="crossword")h+=crossword(t); if(m==="exam")h+=exam(t); if(m==="mini")h+=mini(t); document.getElementById('content').innerHTML=h; updateScore(); if(m==="memory")initMemory(t); if(m==="training")initLetterTasks();}
